...
namespace DevBasics.CarManagement
{
    public class CarManagementService : ICarManagementService
    {
        ...
        private readonly HttpHeaderSettings httpHeader;
        private readonly IBulkRegistrationService bulkRegisterService;
        private readonly ICarRegistrationRepository carRegistrationRepository;
        private readonly ILeasingRegistrationRepository leasingRegistrationRepository;
        private readonly IEnumerable<ICarPoolNumbeGenerator> carPoolNumberHelpers;
        ...
        public CarManagementService(
            ...
            IEnumerable<ICarPoolNumbeGenerator> carPoolNumberHelpers)
        {
            ...
            this.leasingRegistrationRepository = leasingRegistrationRepository;
            this.carPoolNumberHelpers = carPoolNumberHelpers;
        }
        ...
        public async Task<ServiceResult> RegisterCarsAsync(RegisterCarsModel registerCarsModel, bool isForcedRegistration, Claims claims, string identity = "Unknown")
        {
            ...
            try
            {
                ...
                carPoolNumberHelpers.FirstOrDefault(x => x.GetType() == typeof(ToyotaCarPoolNumberGenerator))?.Generate(registerCarsModel.Cars.FirstOrDefault().CarPool,
                    out registrationId,
                    out carPoolNumber);
                ...
                if (registrationResult.RegisteredCars != null && registrationResult.RegisteredCars.Count > 0)
                {
                    ...
                    if (!hasMissingData)
                    {
                        ...
                        try
                        {
                            ...
                            apiTransactionResult = await bulkRegisterService.ExecuteRegistrationAsync(requestPayload);
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine(
                ...
                else
                {
                    ...
                    foreach (IGrouping<string, CarRegistrationModel> grp in group)
                    {
                        ...
                        foreach (CarRegistrationModel dbApiCar in dbApiCars)
                        {
                            ...
                            await new CarRegistrationRepository(leasingRegistrationRepository, bulkRegisterService, _mapper).UpdateRegisteredCarAsync(dbCar, identity);
                        }
                    }
        ...
        public async Task<RequestContext> InitializeRequestContextAsync()
        {
            ...
            try
            {
                ...
                RequestContext requestContext = new RequestContext()
                {
                    ...
                    LanguageCode = LanguageCodesProvider.GetLanguageCodes()["English"],
